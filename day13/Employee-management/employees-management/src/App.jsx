...
const BASE_EMPLOYEES = [
  ...
  { id: 7, name: 'Bala',           role: 'Software Engineer',  department: 'Engineering', status: 'Active',   email: 'bala@company.com',           joiningDate: '2024-01-10' },
  { id: 8, name: 'Asbar',          role: 'UI Designer',         department: 'Design',      status: 'Active',   email: 'asbar@company.com',          joiningDate: '2024-02-15' },
  { id: 9, name: 'Nithis',         role: 'Data Analyst',       department: 'Analytics',   status: 'Active',   email: 'nithis@company.com',         joiningDate: '2024-03-20' },
  { id: 10, name: 'Kamalesh',      role: 'HR Specialist',      department: 'People',      status: 'Active',   email: 'kamalesh@company.com',       joiningDate: '2024-04-05' }
...
const calcPaySlip = (salary) => {
  ...
  const transport = Math.round(salary * 0.10)
  const special   = salary - basic - hra - transport
  const grossPay  = salary
  const pfDed     = Math.round(basic * 0.12)
  const profTax   = 200
  const incomeTax = Math.round(salary * 0.05)
  const totalDed  = pfDed + profTax + incomeTax
  const netPay    = grossPay - totalDed
  return { basic, hra, transport, special, grossPay, pfDed, profTax, incomeTax, totalDed, netPay }
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        console.error(err)
        setApiError('Could not reach backend API. Showing sample data.')
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (res.data.role === 'employee') {
          setLoggedInEmployee(res.data.user)
        } else {
  ...
  const handleAddEmployee = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/api/employees`, {
      ...
      if (res.data.success) {
        // Refresh employees, attendance, and salary records from API
        const empsRes = await axios.get(`${API_BASE_URL}/api/employees`)
        setEmployees(empsRes.data)
        const salRes = await axios.get(`${API_BASE_URL}/api/salary`)
      ...
      console.error(err)
      alert("Failed to add employee via API")
    }
  }
  ...
  const handleAttendanceMark = async (empId, status) => {
    try {
      ...
      console.error(err)
      alert("Failed to update attendance via API")
    }
  }
  ...
  const handleScheduleAdd = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/api/schedule`, {
        ...
        when: newScheduleItem.when.trim()
      })
      if (res.data.success) {
      ...
      console.error(err)
      alert("Failed to add schedule item via API")
    }
  }

  const handleMarkPaid = async (empId) => {
    try {
      const res = await axios.post(`${API_BASE_URL}/api/salary/pay`, {
        empId,
        month: todayMonth,
        year: todayYear
      })
      if (res.data.success) {
        // Refresh salary records from API
        const salRes = await axios.get(`${API_BASE_URL}/api/salary`)
        setSalaryRecords(salRes.data)
      }
    } catch (err) {
      console.error(err)
      alert("Failed to mark salary paid via API")
    }
  }
  ...
  const handleSalaryUpdate = async (empId, newSalary) => {
    ...
    try {
      const res = await axios.put(`${API_BASE_URL}/api/employees/${empId}/salary`, {
        salary: sal
      })
      if (res.data.success) {
        // Refresh employees and salary records from API
        const empsRes = await axios.get(`${API_BASE_URL}/api/employees`)
        setEmployees(empsRes.data)
        const salRes = await axios.get(`${API_BASE_URL}/api/salary`)
        setSalaryRecords(salRes.data)
      ...
      console.error(err)
      alert("Failed to update salary via API")
    }
  }
      ...
      <div className="pay-slip-card">
        ...
        <div className="pay-slip-earnings-wrap">
          <table className="pay-slip-breakdown-table">
            ...
            <tbody>
              <tr>
                <td>Basic Pay <span className="ps-pct">(50%)</span></td>
                <td className="ps-amt">{formatCurrency(ps.basic)}</td>
                <td>Provident Fund <span className="ps-pct">(12% of Basic)</span></td>
                <td className="ps-deduct">{formatCurrency(ps.pfDed)}</td>
              </tr>
              <tr>
                <td>House Rent Allow. <span className="ps-pct">(20%)</span></td>
                <td className="ps-amt">{formatCurrency(ps.hra)}</td>
                <td>Professional Tax</td>
                <td className="ps-deduct">{formatCurrency(ps.profTax)}</td>
              </tr>
              <tr>
                <td>Transport Allow. <span className="ps-pct">(10%)</span></td>
                <td className="ps-amt">{formatCurrency(ps.transport)}</td>
                <td>Income Tax <span className="ps-pct">(5%)</span></td>
                <td className="ps-deduct">{formatCurrency(ps.incomeTax)}</td>
              </tr>
              <tr>
                <td>Special Allowance</td>
                <td className="ps-amt">{formatCurrency(ps.special)}</td>
                <td></td>
                <td></td>
              </tr>
            </tbody>
            <tfoot>
              <tr className="ps-total-row">
                ...
                <td className="ps-amt ps-total">{formatCurrency(ps.grossPay)}</td>
                <td><strong>Total Deductions</strong></td>
                <td className="ps-deduct ps-total">{formatCurrency(ps.totalDed)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        <div className="pay-slip-footer-row">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            ...
            <span className={`sal-badge ${salRecord?.status?.toLowerCase() || 'pending'}`}>
              {salRecord?.status === 'Paid' ? '✅' : '⏳'} {salRecord?.status || 'Pending'}
            </span>
            {salRecord?.paidOn && (
              <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>· Paid on {salRecord.paidOn}</span>
            )}
          </div>
          ...
          {showCreds && (
            <div className="cred-box" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div className="cred-row" style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: 10, display: 'block' }}>
                <span className="cred-role" style={{ display: 'block', marginBottom: 5 }}>👤 Employees</span>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '4px', fontSize: '0.8rem', textAlign: 'left' }}>
      ...
      <main className="portal-main">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            {role === 'admin' && (
              <button type="button" className="export-excel-btn" title="Export all data to Excel" onClick={handleExportExcel}>
                📥 Export Excel
              </button>
            )}
        ...
        <div className="page-content">
            ...
            <div className="panel-grid">
              <div className="panel-card">
                <div className="panel-header">
                  <h3>👥 All Employees ({filteredEmployees.length}{searchQuery ? ` of ${employees.length}` : ''})</h3>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
                          ...
                          {role === 'admin' && (
                            <div className="emp-card-actions">
                              ...
                              <button type="button" className="slip-mini-btn" title="View Pay Slip"
                                onClick={() => { setPaySlipEmployee(emp); setShowPaySlipModal(true) }}>🧾</button>
                              <button type="button" className="delete-mini-btn" title="Delete Employee"
                                onClick={() => handleDeleteEmployee(emp.empId)}>🗑️</button>
                            </div>
                          )}
                        </div>
                      </div>
          ...
          {activeSection === 'salary' && (
            <>
                            ...
                            <tr key={emp.empId}>
                              ...
                              <td>
                                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                                  ...
                                  <button type="button" className="view-slip-btn"
                                    onClick={() => { setPaySlipEmployee(emp); setShowPaySlipModal(true) }}>
                                    🧾 Slip
                                  </button>
                                </div>
                              </td>
              ...
              {/* Employee Professional Pay Slip */}
              {role === 'employee' && myEmployee && (
                <div>
                  {renderPaySlip(myEmployee, myEmpSalaryRecord)}
                  <div style={{ display: 'flex', gap: 12, marginTop: 16, flexWrap: 'wrap' }}>
                    <button type="button" className="primary-btn" onClick={() => window.print()}>🖨️ Print Pay Slip</button>
                  </div>
        ...
        <div className="payslip-modal-overlay" onClick={() => setShowPaySlipModal(false)}>
          <div className="payslip-modal-content" onClick={e => e.stopPropagation()}>
            <div className="payslip-modal-header">
              <h3>🧾 Pay Slip — {paySlipEmployee.name}</h3>
              <button type="button" className="chat-close" onClick={() => setShowPaySlipModal(false)}>✕</button>
            </div>
            ...
            <div className="payslip-modal-actions">
              <button type="button" className="primary-btn" onClick={() => window.print()}>🖨️ Print</button>
              <button type="button" className="logout-btn" style={{ minWidth: 100 }} onClick={() => setShowPaySlipModal(false)}>✕ Close</button>
            </div>
          </div>
        </div>
      )}
